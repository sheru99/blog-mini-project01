...
const authorCreate = async function (req, res) {
  try {
    ...
    if (!["Mr", "Mrs", "Miss"].includes(title)) {
      return res
        .status(400)
        .send({ status: false, msg: `required valid title ==> ${title} ` });
    }
    ...
    if (Object.keys(rest).length > 0)
      return res
        .status(400)
        .send({ status: false, msg: `Invalid key => ${Object.keys(rest)}` });
    ...
    if (Object.keys(req.body).length != 0) {
      ...
      if (!fname) {
        return res
          .status(400)
          .send({ status: false, msg: " Fname field is requires" });
      }
      if (!lname) {
        return res
          .status(400)
          .send({ status: false, msg: "lname field is required" });
      }
      if (!title) {
        return res
          .status(400)
          .send({ status: false, msg: "title field is requried" });
      }
      ...
      if (!password) {
        return res
          .status(400)
          .send({ status: false, msg: " password field is requires" });
      }
      ...
      if (!validator.isEmail(req.body.email)) {
        return res
          ...
          .send({
            status: false,
            msg: "your email id is not in proper format ",
          });
...
const loginAuthor = async function (req, res) {
  try {
    ...
    if (!validator.isEmail(req.body.email)) {
      return res
        .status(400)
        .send({ status: false, msg: "your email id is not in proper format " });
    }
